        ...
        if (meta.kind === 'hasMany') {
          if (payload.hasOwnProperty(key)) {
            ...
            if (_this.hasDeserializeRecordsOption(key)) {
              if (typeof payload[key] === 'object' && !_ember['default'].isArray(payload[key])) {
                payload[key] = Object.keys(payload[key]).map(function (id) {
                  return (0, _lodashObjectAssign['default'])({ id: id }, payload[key][id]);
                });
              } else if (_ember['default'].isArray(payload[key])) {
